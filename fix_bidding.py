...
content = re.sub(r'className="bidding-view"', '', content)
content = re.sub(r'className="glass-panel".*?>', 'className="bg-surface-container-lowest rounded-2xl overflow-hidden shadow-sm border border-outline-variant relative">', content)
content = re.sub(r'<table className="dense-grid">', '<table className="w-full text-left border-collapse">', content)
...
content = re.sub(r'className="title-cell"', 'className="font-[600] leading-snug text-on-surface"', content)
...
with open('frontend/src/views/BiddingView.tsx', 'w') as f:
    f.write(content)
