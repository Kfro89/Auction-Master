...
async def test_public_surplus_login():
    db = SessionLocal()
    try:
        ...
        if not session_cookie:
            print("❌ No Public Surplus session cookie found in settings.")
            return

        print(f"Testing Public Surplus login with cookie (length: {len(session_cookie)})...")
...
if __name__ == "__main__":
    asyncio.run(test_public_surplus_login())
